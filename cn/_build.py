...
# -*- coding: utf-8 -*-
"""组装《代码壁垒》成品:
  1) 合并单一 Markdown 档  代码壁垒_全书.md
  2) 图文 HTML(侧边子目录)  代码壁垒.html
版面沿用《代理觉醒》_build.py(同一套 CSS / 解析器),
内容为技术评注书(蒸馏、代理操作与缓存的工程实录)。
...
BOOK_TITLE = "代码壁垒"
BOOK_SUB   = "蒸馏、代理操作与缓存的工程实录 · The Code Barrier"
COVER_CANDIDATES = ["TheCodeBarrier.png", "CodeBarrier.png", "代码壁垒.png", "cover.png"]
...
def build_merged():
    ...
    cp = cover_path()
    if cp:
        parts.append(f"![{BOOK_TITLE} — 封面海报]({os.path.basename(cp)})\n")
    parts.append("> 全书合并版。本书为教育与研究用途，横跨蒸馏、代理操作与缓存三大主题"
                 "的整理与批判性评注。\n")
    parts.append("\n---\n\n## 目录\n")
    for base in FILES:
...
def md_to_html(text):
    ...
    while i < n:
        ...
        if ln.startswith(">"):
            ...
            if any("🔍" in b for b in blk):       # 高端点评框(蓝绿)
                cls = "review"
            elif any(("🧠" in b or "💡" in b or "⚠️" in b) for b in blk):  # 观念/提醒框(金)
                cls = "concept"
            else:
                cls = "quote"
            inner, buf = [], []
...
def cover_section():
    ...
    fig = (f'<section class="chapter cover-page" id="cover">'
           ...
           f'alt="{html.escape(BOOK_TITLE)} — 封面海报"/></figure></section>')
