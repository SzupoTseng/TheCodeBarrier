#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Assemble the Japanese edition of "The Code Barrier":
  1) merged single Markdown  TheCodeBarrier_full.md
  2) illustrated HTML (sidebar TOC)  TheCodeBarrier.html
Same CSS / parser as the English _build.py; Japanese metadata + regexes + CJK fonts.
...
BOOK_TITLE = "コードの壁"
BOOK_SUB   = "蒸留・エージェント・キャッシュの工学 — The Code Barrier"
OUTFILE    = "TheCodeBarrier"
COVER_CANDIDATES = ["TheCodeBarrier.png", "CodeBarrier.png", "cover.png"]
...
def title_for(base, text):
    if base.startswith("00_"):
        ...
        return m.group(1).strip() if m else "序"
    m = re.search(r'^#\s+(第\d+章.*)$', text, re.M)
    if m: return m.group(1).strip()
    m = re.search(r'^#\s+(付録[A-R].*)$', text, re.M)
    return m.group(1).strip() if m else base

def part_for(text):
    m = re.search(r'^#\s+(第[一二三四五六七八九十]+部.*)$', text, re.M)
...
def build_merged():
    ...
    cp = cover_path()
    if cp:
        parts.append(f"![{BOOK_TITLE} — 表紙ポスター]({os.path.basename(cp)})\n")
    parts.append("> 合本版。本書は教育・研究を目的とする——蒸留・エージェントによるコンピュータ操作・"
                 "キャッシュという三つの主題にまたがる整理と批判的注釈である。\n")
    parts.append("\n---\n\n## 目次\n")
    for base in FILES:
...
body{margin:0;font-family:"Hiragino Mincho ProN","Yu Mincho","YuMincho","Noto Serif JP","Noto Serif CJK JP","Iowan Old Style","Palatino Linotype",Georgia,serif;color:var(--ink);background:var(--pa);line-height:1.85;font-size:clamp(16.5px,2.7vw,18px);overflow-wrap:break-word}
#wrap{display:flex;align-items:flex-start}
#side{position:sticky;top:0;height:100vh;overflow-y:auto;width:320px;min-width:320px;background:#14182a;color:#cdd2e6;padding:26px 18px;font-family:"Hiragino Kaku Gothic ProN","Yu Gothic","Meiryo","Noto Sans JP","Helvetica Neue",Arial,sans-serif;font-size:13.5px;line-height:1.5}
...
h1{font-size:29px;letter-spacing:.5px;line-height:1.4;margin:8px 0 18px}
...
code{background:#eee;padding:1px 5px;border-radius:4px;font-size:.9em;font-family:"SF Mono",Consolas,"Yu Gothic",monospace}
pre.code{background:#14182a;color:#cdd2e6;padding:16px 18px;border-radius:10px;overflow-x:auto;font-size:13px;line-height:1.5;font-family:"SF Mono","DejaVu Sans Mono",Consolas,"Yu Gothic",monospace}
pre.code code{background:none;padding:0;color:inherit}
a{color:var(--accent2)}
hr{border:0;border-top:1px solid var(--line);margin:36px 0}
ul,ol{margin:14px 0;padding-left:24px}
li{margin:6px 0}
table{border-collapse:collapse;width:100%;margin:20px 0;font-size:14.5px;font-family:"Hiragino Kaku Gothic ProN","Yu Gothic","Meiryo","Noto Sans JP","Helvetica Neue",Arial,sans-serif}
...
figcaption{font-size:13px;color:var(--mut);margin-top:10px;font-family:"Hiragino Kaku Gothic ProN","Yu Gothic","Meiryo","Noto Sans JP","Helvetica Neue",Arial,sans-serif;line-height:1.5}
...
blockquote.concept .box-title{font-size:17px;font-weight:700;color:#9a6a1a;font-family:"Hiragino Kaku Gothic ProN","Yu Gothic","Meiryo","Noto Sans JP","Helvetica Neue",Arial,sans-serif;margin:0 0 10px}
...
blockquote.review .box-title{font-size:16px;font-weight:700;color:#2f5d7a;font-family:"Hiragino Kaku Gothic ProN","Yu Gothic","Meiryo","Noto Sans JP","Helvetica Neue",Arial,sans-serif;margin:0 0 10px}
...
def cover_section():
    ...
    fig = (f'<section class="chapter cover-page" id="cover">'
           ...
           f'alt="{html.escape(BOOK_TITLE)} — 表紙ポスター"/></figure></section>')
    return '<a href="#cover">表紙</a>', fig
